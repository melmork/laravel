...
;(function($) {
	...
	function Sequence(element, options, defaults, get) {
		...
		var preloadTheseImagesLength = self.settings.preloadTheseImages.length; //how many single img to load?
		...
		function saveImagesToArray(length, srcOnly) {
			var imagesToPreload = []; //saves the img that are to be preloaded
			if(!srcOnly){
				for(var i = length; i > 0; i--){ //for each frame to be preloaded...
					self.frames.eq(self.settings.preloadTheseFrames[i-1]-1).find("img").each(function() { //find <img>'s in specific frames, and for each found...
						imagesToPreload.push($(this)[0]); //add it to the array of img to be preloaded
					});
				}
			}else{
				for(var j = length; j > 0; j--) { //for each frame to be preloaded...
					imagesToPreload.push($("body").find('img[src="'+self.settings.preloadTheseImages[j-1]+'"]')); //find any <img> with the given source and add it to the array of img to be preloaded
				}
			}
		...
		function imagesLoaded(imagesToPreload, callback) {
			...
			function imgLoaded( img, isBroken ) {
				...
				loaded.push(img); // store element in loaded img array

				if(isBroken) { // keep track of broken and properly loaded img
					broken.push(img);
				}else{
					proper.push(img);
				}
				...
				if($images.length === loaded.length) { // call doneLoading and clean listeners if all img are loaded
					setTimeout(doneLoading);
					$images.unbind('.imagesLoaded');
				}
			}

			if(!$images.length) { // if no img, trigger immediately
				doneLoading();
			}else{
				$images.bind('load.imagesLoaded error.imagesLoaded', function(event) {
					...
					// cached img don't fire load sometimes, so we reset src, but only when dealing with IE, or image is complete (loaded) and failed manual check webkit hack from http://groups.google.com/group/jquery-dev/browse_thread/thread/eee6ab7b2da50e1f
		...
		if(self.settings.preloader !== undefined && self.settings.preloader !== false && (preloadTheseFramesLength !== 0 || preloadTheseImagesLength !== 0)) { //if using the preloader and the dev has specified some img should preload...
			var frameImagesToPreload = saveImagesToArray(preloadTheseFramesLength); //get img from particular Sequence frames to be preloaded
			var individualImagesToPreload = saveImagesToArray(preloadTheseImagesLength, true); //get img with specific source values to be preloaded
			var imagesToPreload = $(frameImagesToPreload.concat(individualImagesToPreload)); //combine frame img and individual img
	...
	var defaults = {
		...
		//Preloader Settings
		preloader: false,
		preloadTheseFrames: [1], //all img in these frames will load before Sequence initiates
		preloadTheseImages: [ //specify particular img to load before Sequence initiates
			/* Example usage
			"img/catEatingSalad.jpg",
			"img/meDressedAsBatman.png"
			*/
		],
		/*Note: You can use preloadTheseFrames and preloadTheseImages together. You might want to load all img in frame 1 and just one big image from frame 2 for example*/
